...
*/
// 正常的64位系统lua的int类型是long long类型, float类型是double
// LUA_FLOAT_TYPE目前没有可能是LUA_FLOAT_LONGDOUBLE
...
*/
#define LUA_VDIR	LUA_VERSION_MAJOR "." LUA_VERSION_MINOR // 5.3
#if defined(_WIN32)	/* { */
/*
...
#define LUA_ROOT	"/usr/local/" // /usr/local/
#define LUA_LDIR	LUA_ROOT "share/lua/" LUA_VDIR "/" // /usr/local/share/lua/5.3/
#define LUA_CDIR	LUA_ROOT "lib/lua/" LUA_VDIR "/" // /usr/local/lib/lua/5.3/
// lua中执行 print(package.path)可以看到 LUA_PATH_DEFAULT
// LUA_PATH_DEFAULT 是lua模块的默认搜索路径
...
// 如果修改了LUA_FLOAT_TYPE(默认为double) 那么标准库内的函数就要修改
...
// %ld=long
// %lld=long long
// 这里LUA_INTEGER_FMT解析成 "%" "ll" "d"

#define LUA_INTEGER_FMT		"%" LUA_INTEGER_FRMLEN "d"

// LUAI_UACINT是LUA_INTEGER默认参数提升的结果
...
// LUA_KCONTEXT是intptr_t
// intptr_t实际上就是一个long或int 要和指针长度一样,目的是使用整数来存一个地址
